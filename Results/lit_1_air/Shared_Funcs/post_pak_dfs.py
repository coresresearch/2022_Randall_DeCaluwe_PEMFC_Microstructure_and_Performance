...
def df_tool(df_i,df_p,ca,gdl,cl,i_find,yamlfile):
    ...
    if i_find != 0:
        ...
        for i in range(cl.d['Ny']):
            ...
            ca.pt_s[i].electric_potential = 0
            
            ca.naf_b[i].electric_potential = -sv[ca.ptr['phi_dl'] +mvr]
            ca.naf_gs[i].electric_potential = -sv[ca.ptr['phi_dl'] +mvr]
            ca.naf_ws[i].electric_potential = -sv[ca.ptr['phi_dl'] +mvr]
            
            ca.naf_b[i].Y = sv[ca.ptr['rho_naf_k'] +mvr]
            ca.pt_s[i].coverages = sv[ca.ptr['theta_pt_k'] +mvr]
            
            i_far_frac[i] = -ca.pt_s[i].get_net_production_rates(ca.carb)*ct.faraday\
                          *cl.d['SApv_pt'] / (df_row.iloc[0,0] *cl.d['1/dy'] *100**2)
