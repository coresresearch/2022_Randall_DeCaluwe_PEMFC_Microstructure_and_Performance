...
def sig_io_func(p, ca, sv):
    ...
    elif p['sig_method'] == 'lit': sig_io = sig_io_bulk /0.672
...
def rxn_areas_cs(w_Pt, t_cl, eps_gas, t_naf, r_c, r_Pt, rho_Pt, theta):
    ...
    m_Pt_part = w_Pt / t_cl / (1 - eps_gas) *V_part
    ...
    if SA_naf_int < SA_naf_edit:
        SA_pv_naf = (1 - eps_gas)*SA_naf_int / V_part
        p_eff_SAnaf = 1.0
    else:
        SA_pv_naf = (1 - eps_gas)*SA_naf_edit / V_part
