...
i_ext0 = np.linspace(1e-3,0.3,15) # currents in kinetic region [A/cm^2]
i_ext1 = np.linspace(5e-1,1.9,15) # currents in ohmic region [A/cm^2]
i_ext2 = np.linspace(2.00,2.5,35) # currents near limitting [A/cm^2]
...
t_naf = np.ones(5)*18e-9        # thickness of Nafion shells [m]
...
post_only = 0       # only run post-processing (1:current, 2:past)
...
save_folder = 't18_pt_eql_025_15um_cl' # folder name for saving files
