...
i_ext1 = np.linspace(5e-1,2.0,15) # currents in ohmic region [A/cm^2]
i_ext2 = np.linspace(2.2,2.85,20) # currents near limitting [A/cm^2]
...
w_Pt = 0.025                    # Pt loading per geometric area [mg/cm^2]
...
t_naf = np.array([5,5,7,7,12])*1e-9        # thickness of Nafion shells [m]
pt_dist = 'lin'                 # CL distribution of pt: 'lin', 'exp', 'equal'
...
post_only = 1       # only run post-processing (1:current, 2:past)
debug  = 0          # plot solution vector variables vs time (1:low, 2:high)
grads  = 0          # plot solution vector depth gradients (1:low, 2:high)
radial = 0          # plot solution vecotr radial gradients (1:low, 2:high)
polar  = 2          # plot full cell polarization curve (1:w/o pwr, 2:w/ pwr)
over_p = 0          # plot overpotential curve

" Verification settings "
data = 'air'        # include data from Owejan et. al. if (0: off, 1: on)
i_ver = 1           # verify current between GDL and CL (0: off, 1: on)
i_find = 2.0        # current to use in i_ver calculation [A/cm^2]

" Save/load options "
load_folder = 'folder_name'          # folder name to load past dfs
save_folder = 't_inc_pt_lin_025_15um_cl'          # folder name for saving files
save = 1                             # toggle saving on/off
...
# Current lit relation (w/ dependence on rho_naf_h2o) best fit is chi2 = 2.31
# Current lit relation (w/ only dependence on gas RH) best fit is chi2 = 3.92
