...
%The NR and conductivity measurements used to develop the structure-property relationships were performed with Nafion on silicon substrates. In a PEMFC CL, Nafion binds to carbon and Pt rather than silicon. Others have studied Nafion on Pt and carbon~\cite{bib:wood_2009, bib:shrivastava_2018, bib:dura_2009, bib:ito_2020}, but fewer details are included, providing no clear path to directly capture transport property variations with varying Nafion thickness. Instead, we propose two hypothetical structural approximations, illustrated in figure~\ref{fig:struct-models}. These approximations are informed by studies showing that Nafion at Pt and carbon interfaces tends to have less structure than at silicon interfaces~\cite{bib:wood_2009, bib:ito_2020, bib:dura_2009, bib:shrivastava_2018}. In some cases, the Nafion structure is homogeneous (i.e. the water uptake is uniformly distributed throughout the film). This is likely to occur at carbon-Nafion interfaces due to carbon being more hydrophobic and rougher than silicon. \crr{The `uniform' method (figure~\ref{fig:struct-models}(a)) considers the Nafion ionomer water distribution to be spatially uniform, with water uptake equal to that measured by NR for films of similar thickness. The `mixed' method (figure~\ref{fig:struct-models}(b)) assumes that Nafion has different structures at carbon versus Pt interfaces. Since Nafion at \emph{pure} carbon or Pt interfaces is differently structured, we assume that Nafion on a Pt-decorated carbon substrate may exist in both layered and unlayered regions. The relative occurrence of each structure is dependent upon the relative carbon-Nafion and Pt-Nafion interfacial areas. Layered interfaces near Pt catalysts include water-rich and water-poor regions, as illustrated in figure~\ref{fig:struct-models}(c). This layering is consistent with reported observations at Pt-Nafion interfaces~\cite{bib:wood_2009, bib:dura_2009, bib:shrivastava_2018}. For the remainder of this publication these methods are referred to as `uniform' and `mixed'.}
...
%; however, the Pt loading of their cells was not held constant. Instead, their Pt loading linearly increased with increasing CL thickness. Here, we predict that this trend is still relevant when considering a constant low Pt loading. 
...
In this section, detailed structure-property relationships, updated during the simulation as a function of time and location, allow simulation of non-uniform Nafion and Pt loadings. \crr{Results in all previous sections} assume uniform Nafion and Pt loading as a function of CL depth. %In actual PEMFCs, the Nafion thickness is heterogeneous and ranges between roughly 5 and 15~nm. Similarly, Pt is not uniformly dispersed, and can form aggregates and clusters within CL regions. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Nomenclature
\nomenclature[Aa]{$A$}{specific area [m$^2$~m$^{-3}$]}
\nomenclature[Acac]{$C_{\rm ac}$}{activity concentration [kmol~m$^{-2}$, kmol~m$^{-3}$]}
\nomenclature[Acdl]{$C_{\rm dl}$}{specific double layer capacitance [F~m$^{-2}$]}
\nomenclature[Acw]{$C_{\rm w}$}{molar density of liquid phase [kmol~m$^{-3}$]}
\nomenclature[Ad]{$D$}{diffusion coefficient [m$^2$~s$^{-1}$]}
\nomenclature[Af1]{$f$}{scaling factor [-]}
\nomenclature[Af2]{$F$}{Faraday's constant [C~kmol$^{-1}$]}
\nomenclature[Aghor]{$\Delta g$}{Gibbs free energy [J~kmol$^{-1}$]}
\nomenclature[Aj1]{$j$}{current [A~m$^{-3}$ or A~m$^{-2}$]}
\nomenclature[Aj2]{$\mathbf{j}$}{current density [A~m$^{-2}$]}
\nomenclature[Aj3]{$\mathbf{J}$}{mass flux [kg~m$^{-2}$~s$^{-1}$]}
\nomenclature[Ak]{$K$}{permeability [m$^2$]}
\nomenclature[Anpts]{$n_{\rm pts}$}{number of data points [-]}
\nomenclature[An]{$\mathbf{N}$}{molar flux [kmol~m$^{-2}$~s$^{-1}$]}
\nomenclature[Ap]{$P$}{pressure [Pa]}
\nomenclature[Ar]{$r$}{radius [m]}
\nomenclature[Ara]{$R$}{resistance [$\Omega$~cm$^2$]}
\nomenclature[Arbar]{$\Bar{R}$}{universal gas constant [J~kmol$^{-1}$~K$^{-1}$]}
\nomenclature[Asdot]{$\dot{s}$}{molar production rate [kmol~m$^{-2}$~s$^{-1}$]}
\nomenclature[As]{$\mathcal{S}$}{saturation [-]}
\nomenclature[Asd]{\rm SD}{standard deviation [units]}
\nomenclature[At]{$t$}{time [s]}
\nomenclature[Ati]{$t_i$}{thickness of $i$ [m]}
\nomenclature[Atemp]{$T$}{temperature [K]}
\nomenclature[Avhat]{$\hat{V}$}{molar volume [m$^3$~kmol$^{-1}$]}
\nomenclature[Av]{$V_{\rm cell}$}{voltage [V]}
\nomenclature[Aw]{$W$}{molecular weight [kg~kmol$^{-1}$]}
\nomenclature[Ay1]{$y$}{cathode depth [m]}
\nomenclature[Ay2]{$Y$}{mass fraction [-]}

\nomenclature[Balpha]{$\alpha$}{contact angle [degrees]}
\nomenclature[Bbeta]{$\beta$}{symmetry factor for Butler-Volmer [-]}
\nomenclature[Bchi]{$\chi^2$}{goodness-of-fit statistic [-]}
\nomenclature[Bepsilon]{$\epsilon$}{volume fraction [-]}
\nomenclature[Beta]{$\eta$}{overpotential [V]}
\nomenclature[Bgamma1]{$\gamma$}{water surface tension [N~m$^{-1}$]}
\nomenclature[Bgamma2]{$\Gamma$}{adsorption site density [kmol~m$^{-2}$]}
\nomenclature[Blambda]{$\lambda$}{local water uptake [-]}
\nomenclature[Bmu]{$\mu$}{viscosity [Pa~s]}
\nomenclature[Bnu]{$\nu$}{stoichiometric coefficient [-]}
\nomenclature[Bphi]{$\phi$}{electric potential [V]}
\nomenclature[Brho]{$\rho$}{mass density [kg~m$^{-3}$]}
\nomenclature[Bsigma]{$\sigma$}{conductivity [S~m$^{-1}$]}
\nomenclature[Btau]{$\tau_{\rm fac}$}{tortuosity factor [-]}
\nomenclature[Btheta]{$\theta$}{surface coverage [-]}

\nomenclature[Can]{\rm an}{anode}
\nomenclature[Cav]{\rm av}{average}
\nomenclature[Cbulk]{\rm bulk}{related to bulk-like Nafion}
\nomenclature[Cc]{\rm C}{carbon}
\nomenclature[Cca]{\rm ca}{cathode}
\nomenclature[Ccell]{\rm cell}{related to the full cell}
\nomenclature[Ccl]{\rm CL}{catalyst layer}
\nomenclature[Cdata]{\rm dat}{data}
\nomenclature[Cdl]{\rm dl}{double layer}
\nomenclature[Ceff]{\rm eff}{effective}
\nomenclature[Ceq]{\rm eq}{equilibrium}
\nomenclature[Cext]{\rm ext}{external}
\nomenclature[Cfar]{\rm Far}{Faradaic}
\nomenclature[Cfwd]{\rm fwd}{forward}
\nomenclature[Cg]{\rm g}{gas phase}
\nomenclature[Chor]{\rm HOR}{hydrogen oxidation reaction}
\nomenclature[Ci]{$i$}{reactions index}
\nomenclature[Cio]{\rm io}{ionic transport}
\nomenclature[Ck]{$k$}{species index}
\nomenclature[Clit]{\rm lit}{literature properties, see section~\ref{sect:struct-prop-relationships}}
\nomenclature[Cm]{$m$}{interphase index}
\nomenclature[Cmixed]{\rm mixed}{mixed properties, see section~\ref{sect:struct-prop-relationships}}
\nomenclature[Cmod]{\rm mod}{model}
\nomenclature[Cn]{\rm N}{Nafion phase}
\nomenclature[Cohm]{\rm Ohm}{Ohmic}
\nomenclature[Cp]{$p$}{products index}
\nomenclature[Cpem]{\rm PEM}{proton exchange membrane}
\nomenclature[Cpt]{\rm Pt}{platinum}
\nomenclature[Cr]{$r$}{reactants index}
\nomenclature[Cref]{\rm ref}{reference}
\nomenclature[Crev]{\rm rev}{reverse}
\nomenclature[Ctot]{\rm tot}{total}
\nomenclature[Cuniform]{\rm uniform}{uniform properties, see section~\ref{sect:struct-prop-relationships}}
\nomenclature[Cw]{\rm w}{liquid phase}

\crr{\printnomenclature}
